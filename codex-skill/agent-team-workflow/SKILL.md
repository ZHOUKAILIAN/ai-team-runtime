...
description: Use when the user wants a requirement executed through the Agent Team single-session state machine, especially with /agent-team-run, 执行这个需求：, or equivalent workflow triggers.
---
...
- Normal AI Q&A style development remains valid for one-off edits, investigations, and ad hoc work that do not need the session/state-machine layer.
...
- `scripts/agent-team-run.sh`: skill-bundled runtime-driver helper; by default it calls `agent-team run-requirement` with the `codex-exec` executor and stops at human gates.
