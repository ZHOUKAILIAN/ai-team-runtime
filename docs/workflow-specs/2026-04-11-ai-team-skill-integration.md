...
这份文档说明如何把 `AI_Team CLI Runtime` 以 skill 的方式接入到 `Codex App`，同时保持 skill 与 runtime 的边界清晰。
...
- skill 负责触发、约束和说明
- `ai-team` 负责 bootstrap、状态推进、人工决策记录和学习回流
...
这是因为 `AI_Team` 不是一个单步工具，而是一套多角色、多阶段、有审批边界的运行时。
...
- 这个 skill 的目标是把用户需求送入 AI_Team workflow
...
1. 保持 `ai-team` 为唯一对外 CLI 入口
2. skill 只负责识别触发意图和说明边界
3. 真实流程推进全部通过 `ai-team` 完成
...
- Codex 能从 skill 正确识别何时使用 `AI_Team` workflow
- Codex 知道 `ai-team` 而不是 skill 本身负责流程推进
