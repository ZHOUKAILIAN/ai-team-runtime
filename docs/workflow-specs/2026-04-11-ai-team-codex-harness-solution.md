...
- 让 `AI_Team` 在 `Codex App` 中可稳定运行
- 让 `ai-team` 成为真正的 harness control plane
- 让 Codex 成为当前 stage 的执行器
- 让最小团队循环可以持续运行并可自我迭代

一句话定义：

`AI_Team` 采用 `Codex-only`、`runtime-first` 的 harness 方案。
...
- 把流程事实来源放到 `ai-team` CLI，而不是放到宿主特性里
...
这层负责把 Codex 的自然语言入口接到 `ai-team` runtime。
...
- 识别 AI_Team workflow 触发意图
- 告诉 Codex 应该使用 `ai-team`
- 解释当前最小循环和边界
...
- 任何 AI_Team 需求先走 `ai-team start-session`
...
对当前项目来说，最重要的不是继续扩写 skill，而是继续强化 `ai-team` 这个 CLI harness。
...
`AI_Team` 在 Codex 下的正确实现方式，不是让 skill 接管流程，而是让 `ai-team` 接管流程，让 Codex 专注执行。
