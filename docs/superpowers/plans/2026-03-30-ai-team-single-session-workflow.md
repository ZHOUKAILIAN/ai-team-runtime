# AI_Team Single-Session Workflow Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace the current deterministic AI_Team execution path with a single-session, file-backed workflow that runs `Product -> CEO approval -> Dev <-> QA -> Acceptance -> human Go/No-Go`, while preserving `superpower` TDD inside `Dev`.
...
- Create: `ai_company/workflow_summary.py`
...
- Modify: `codex-skill/ai-company-workflow/scripts/company-run.sh`
...
Add the workflow summary model in `ai_company/models.py`:
...
Update artifact naming in `ai_company/state.py`:
...
```bash
git add ai_company/models.py ai_company/state.py ai_company/workflow_summary.py tests/test_workflow_summary.py tests/test_state.py
...
Update `SKILL.md` and `codex-skill/ai-company-workflow/SKILL.md` so they:
...
Update `codex-skill/ai-company-workflow/scripts/company-run.sh`:
...
```bash
git add SKILL.md codex-skill/ai-company-workflow/SKILL.md codex-skill/ai-company-workflow/scripts/company-run.sh tests/test_skill_package.py
...
```bash
git add ai_company/review.py ai_company/cli.py tests/test_review.py
...
`python3 -m ai_company start-session --message "执行这个需求：..."`
...
git commit -m "docs: describe the single-session AI_Team workflow"
```
