# Agent Team Enforced Stage Gates Implementation Plan
...
**Tech Stack:** Python 3.13, dataclasses, file-backed JSON state, `unittest`, existing `agent-team` argparse CLI.
...
- Create: `agent_team/gatekeeper.py`
...
- Modify: `README.md` and `docs/workflow-specs/2026-04-17-agent-team-enforced-stage-gates-flow.md`
...
def test_create_stage_run_rejects_existing_active_run(self) -> None:
    from agent_team.state import StateStore, StageRunStateError
...
```bash
/tmp/agent-team-runtime-enforced-venv/bin/python -m unittest tests.test_state.StateTests.test_stage_run_lifecycle_persists_active_candidate tests.test_state.StateTests.test_create_stage_run_rejects_existing_active_run
```
...
Add `GateResult` and `StageRunRecord` to `agent_team/models.py`. Add `StageRunStateError`, `create_stage_run`, `active_stage_run`, `load_stage_run`, `submit_stage_run_result`, and `update_stage_run` to `agent_team/state.py`.
...
```bash
/tmp/agent-team-runtime-enforced-venv/bin/python -m unittest tests.test_gatekeeper
```

Expected: tests fail because `agent_team.gatekeeper` does not exist.
...
```bash
/tmp/agent-team-runtime-enforced-venv/bin/python -m unittest tests.test_cli.CliTests.test_submit_stage_result_requires_active_stage_run tests.test_cli.CliTests.test_acquire_submit_verify_product_moves_to_ceo_wait tests.test_cli.CliTests.test_step_reports_verify_when_candidate_is_submitted
```
...
```text
agent-team step
agent-team acquire-stage-run --session-id <id> [--stage <stage>] [--worker <name>]
agent-team verify-stage-result --session-id <id> [--run-id <run_id>]
```
...
- Modify: `tests/test_cli.py`
- Modify: `README.md`
- Modify: `docs/workflow-specs/2026-04-17-agent-team-enforced-stage-gates-flow.md`
