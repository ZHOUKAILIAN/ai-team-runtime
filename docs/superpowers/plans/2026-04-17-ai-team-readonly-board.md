# AI_Team Read-Only Board Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build a local, read-only AI_Team board that aggregates all workspaces and shows `project -> worktree -> session` progress, active stage runs, gate results, and artifact previews.
...
**Tech Stack:** Python 3.13, dataclasses, `json`, `http.server`, `unittest`, existing `ai-team` argparse CLI, static HTML/CSS/JS with no frontend build chain.
...
- Create: `ai_company/workspace_metadata.py`
...
- Create: `ai_company/board_assets.py`
  - Stores the read-only board HTML as a Python string constant.
- Create: `ai_company/board_server.py`
...
- Modify: `docs/workflow-specs/2026-04-11-ai-team-cli-runtime-usage.md`
  - Adds board commands.
- Modify: `docs/workflow-specs/2026-04-11-ai-team-cli-runtime-flow.md`
...
```bash
/tmp/ai-team-runtime-enforced-venv/bin/python -m unittest tests.test_workspace_metadata
```

Expected: fail with `ModuleNotFoundError: No module named 'ai_company.workspace_metadata'`.
...
In `ai_company/cli.py`, import:

```python
from .workspace_metadata import refresh_workspace_metadata
```

Add this helper near the bottom of `ai_company/cli.py`:
...
```bash
/tmp/ai-team-runtime-enforced-venv/bin/python -m unittest tests.test_cli.CliTests.test_start_session_bootstraps_session_from_raw_message
```
...
```bash
git add .gitignore ai_company/workspace_metadata.py ai_company/cli.py tests/test_workspace_metadata.py
...
class BoardSnapshotTests(unittest.TestCase):
    def test_board_snapshot_groups_sessions_by_project_and_worktree(self) -> None:
        from ai_company.board import build_board_snapshot
        ...
        with TemporaryDirectory(dir=local_temp_dir()) as temp_dir:
            ...
            state_root = codex_home / "ai-team" / "workspaces" / "ai-team-runtime-abc"
    ...
    def test_board_snapshot_falls_back_for_legacy_workspace_without_metadata(self) -> None:
        ...
        with TemporaryDirectory(dir=local_temp_dir()) as temp_dir:
            codex_home = Path(temp_dir)
            state_root = codex_home / "ai-team" / "workspaces" / "legacy-workspace-123"
    ...
    def test_artifact_path_must_be_under_discovered_state_roots(self) -> None:
        from ai_company.board import BoardSnapshot, is_allowed_artifact_path
        from ai_company.state import StateStore

        with TemporaryDirectory(dir=local_temp_dir()) as temp_dir:
            codex_home = Path(temp_dir)
            state_root = codex_home / "ai-team" / "workspaces" / "workspace-a"
            store = StateStore(state_root)
...
```bash
/tmp/ai-team-runtime-enforced-venv/bin/python -m unittest tests.test_board
```

Expected: fail with `ModuleNotFoundError: No module named 'ai_company.board'`.
...
```bash
git add ai_company/board.py tests/test_board.py
...
```bash
/tmp/ai-team-runtime-enforced-venv/bin/python -m unittest tests.test_cli.CliTests.test_board_snapshot_outputs_all_workspace_board_json
```
...
Expected: fail with `ModuleNotFoundError: No module named 'ai_company.board_server'`.
...
```bash
git add ai_company/board_assets.py ai_company/board_server.py ai_company/cli.py tests/test_board_server.py
...
In `tests/test_docs.py`, update `test_readme_documents_ai_team_cli_usage`:
...
```bash
/tmp/ai-team-runtime-enforced-venv/bin/python -m unittest tests.test_docs.DocsTests.test_readme_documents_ai_team_cli_usage tests.test_cli.CliTests.test_cli_help_lists_readonly_board_commands
```
...
In `docs/workflow-specs/2026-04-11-ai-team-cli-runtime-usage.md`, add:
...
In `docs/workflow-specs/2026-04-11-ai-team-cli-runtime-flow.md`, add under "当前事实来源":
...
```bash
git add README.md docs/workflow-specs/2026-04-11-ai-team-cli-runtime-usage.md docs/workflow-specs/2026-04-11-ai-team-cli-runtime-flow.md tests/test_docs.py tests/test_cli.py
