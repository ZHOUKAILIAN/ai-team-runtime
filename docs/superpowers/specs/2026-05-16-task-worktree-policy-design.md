...
5. 旧命名里的 `agent-team/` 和 `.agent-team/` 只差一个点，会让共享控制面和本地运行态的边界继续模糊。

## 目标

- 让 `run` 在默认情况下从干净基线创建任务级独立 branch 和独立 worktree。
- 让新 worktree 的行为尽量接近普通 `git worktree`，不重复制造项目五层文档。
- 让 clean base ref、branch 前缀、worktree 根目录、命名策略成为项目级可配置项。
- 让共享控制面目录和本地运行态目录在命名上明显分离。
- 保持配置本地生效且默认不提交。
...
- 本次不在创建新 worktree 时重新执行 `agent-team init`。
- 本次不重新生成 `agt-control/project/`、`docs/project-runtime/`、`docs/governance/` 等仓库内五层文档。

## 核心决策

### 决策零：目录命名基线切换为 `agt-control/` 与 `.agt/`

从这次设计开始，Agent Team 相关目录名称采用以下新基线：
...
这次命名调整的目的，是避免旧命名只靠一个 `.` 做区分，降低误把“共享控制面”当成本地状态目录的概率。
...
### 决策二：配置文件放在 `.agt/local/worktree-policy.json`
...
### 决策六：新 worktree 不重建项目五层文档

新建任务 worktree 时，runtime 只做“像 git 一样”的 worktree 创建，不额外制造项目正式文档。

也就是说，下面这些仓库内正式内容都直接来自选定 `base_ref` 的 checkout 结果：
...
runtime 在创建 worktree 时：

- 不重新执行 `init`
- 不重跑五层分类初始化
- 不重新生成分类 prompt、classification-run、doc-map、五层目录模板

如果这些文件在 `base_ref` 上本来就存在，它们自然会出现在新 worktree 中；如果 `base_ref` 上没有，也不应该由 worktree 创建流程偷偷补出来。

### 决策七：AGT 本地支持状态要从源工作区带到新 worktree

虽然项目正式文档不重建，但 `agt` 本地支持状态应该与源工作区保持一致的起点，避免新 worktree 像“裸仓库”一样缺少运行配置。

创建新 worktree 时，应该从源工作区的 `.agt/` 复制以下本地支持内容到新 worktree：
...
- 属于本地运行支持或本地学习上下文
- 不属于仓库正式五层文档
- 新 worktree 启动时应该和当前使用中的项目本地环境保持同一起点
...
- `_runtime/`
- `sessions/`
- 已存在 session 的中间产物、trace、运行历史

也就是说，新 worktree 要继承“怎么运行”和“已经学到什么”，但不继承“之前任务跑到哪一步”。
...
1. 从当前项目根读取 `.agt/local/worktree-policy.json`。
2. 如果不存在，加载内建默认配置。
...
7. 将源工作区的 AGT 本地支持状态复制到新 worktree 的 `.agt/`。
8. 将 session 运行环境切换到这个新 worktree 下继续执行。
...
`<worktree>/.agt/local/worktree-policy.json`
...
这里的快照属于“源工作区 AGT 本地支持状态复制”的一部分，而不是新生成一套项目五层文档。
...
- 新目录命名基线是 `agt-control/` 与 `.agt/`。
- 实现阶段必须兼容读取旧的 `agent-team/` 和 `.agent-team/`，避免一次性破坏现有仓库和本地状态。
- 在兼容期内，新的写入目标应优先落到 `agt-control/` 与 `.agt/`，旧路径只作为读取或迁移来源。
...
8. 仓库内五层文档不会在 worktree 创建时被重新生成。
9. `executor-env.json`、`skill-preferences.yaml`、`local/`、`memory/` 会从源工作区复制到新 worktree。
10. `session-index.json`、`_runtime/`、历史 session 产物不会被复制。
11. worktree policy 会被快照复制到新 worktree。
