# AI_Team GitHub Releases Distribution Design

Date: 2026-04-19

## Goal

Ship `AI_Team` as a mature CLI product distributed only through GitHub Releases, without requiring users to clone the repository or install from PyPI.
...
6. expose a stable `ai-team` command under `~/.local/bin`
...
- `ai_company-<version>-py3-none-any.whl`
- `ai_company-<version>.tar.gz`
- `install.sh`
- `SHA256SUMS`
- `CHANGELOG.md`
...
~/.local/bin/
  ai-team -> ~/.local/share/ai-team/current/venv/bin/ai-team
```
...
- the stable command path is always `~/.local/bin/ai-team`
...
-> installer runs ai-team --help smoke check
...
- whether the first release includes a CLI self-management namespace such as `ai-team self doctor`
...
8. installation exposes a working `ai-team` command under a stable user-local path
...
The right first mature distribution model for `AI_Team` is:
