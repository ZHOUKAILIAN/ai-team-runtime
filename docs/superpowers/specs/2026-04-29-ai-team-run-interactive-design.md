# AI_Team Run Interactive Design

## Summary

Add `ai-team run-interactive` as a terminal harness for the existing AI_Team state machine. The command provides a human-friendly CLI flow for collecting a requirement, aligning the requirement and acceptance criteria through `codex exec`, asking the human to confirm that alignment once, then driving the existing Product, Dev, QA, and Acceptance stages through runtime gates.

The harness must not replace the workflow state machine. It coordinates prompts, `codex exec` calls, stage-run acquisition, stage-result submission, verification, and human decisions using the existing `ai-team` runtime APIs.

## Goals

- Let a user run `ai-team run-interactive` from a repository root and be prompted for the requirement.
...
- Keep Product, Dev, QA, Acceptance, artifacts, stage runs, and verification under the existing AI_Team runtime controls.
- Make each model task small enough to be understandable, auditable, and retryable.

## Non-Goals

- Do not make `ai-team` with no subcommand interactive. The new entrypoint is `ai-team run-interactive`.
...
What requirement should AI_Team run?
> Add ...
...
```bash
ai-team run-interactive --message "执行这个需求：..."
ai-team run-interactive --session-id 20260429T114149367856Z-session
ai-team run-interactive --dry-run
```
...
2. If a session was created, it remains inspectable through `ai-team status`, `ai-team resume`, and `ai-team panel`.

## Architecture

Add a small interactive orchestration layer under `ai_company`.

Proposed modules:

- `ai_company/interactive.py`
  - Owns terminal prompts, user choices, flow control, and high-level orchestration.
- `ai_company/codex_exec.py`
  - Wraps `codex exec` invocation, stdout capture, stderr capture, exit-code handling, and optional JSONL parsing.
- `ai_company/stage_harness.py`
  - Runs one AI_Team stage using existing runtime commands and `codex exec`.
- `ai_company/alignment.py`
...
- `ai_company/state.py`
- `ai_company/stage_machine.py`
- `ai_company/stage_contracts.py`
- `ai_company/execution_context.py`
- `ai_company/cli.py`
...
1. `ai-team acquire-stage-run --session-id <id> --stage <stage> --worker codex-exec`
2. `ai-team build-execution-context --session-id <id> --stage <stage>`
...
7. `ai-team submit-stage-result --session-id <id> --bundle <bundle_path>`
8. `ai-team verify-stage-result --session-id <id>`
9. Read `ai-team step --session-id <id>` and continue only according to runtime state.
...
- User interrupt: stop cleanly and print `ai-team resume --session-id <id>`.

## Resume Behavior

`ai-team run-interactive --session-id <id>` resumes from runtime state.
...
- Every prompt sent to `codex exec` is saved under `.ai-team/<session_id>/codex_exec/`.
- Every raw model response is saved under `.ai-team/<session_id>/codex_exec/`.
...
- `ai-team run-interactive --message ...` reaches `WaitForHumanDecision`.
...
```bash
cd /path/to/project
ai-team run-interactive --message "执行这个需求：Add a visible demo change with clear acceptance criteria."
ai-team status
ai-team panel --open-browser
```
...
3. Update generated `ai-team-run` skill text to recommend `run-interactive` for terminal workflows.
