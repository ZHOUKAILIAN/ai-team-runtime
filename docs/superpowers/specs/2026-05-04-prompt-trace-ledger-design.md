...
当前 runtime 已经能把 `session.json`、stage artifact、stage run trace、stdout / stderr 等运行痕迹落盘，但默认 `agent-team run` 的 `codex-exec` stage prompt 只在内存里拼装后直接传给 Codex CLI。这些 prompt 不是统一的 session 级索引，控制台也不能稳定展示。
...
- Prompt trace 能关联回 session 和 stage run。
...
- `agent-team run` 默认 `codex-exec` 生成的 Product / Dev technical plan / Dev / QA / Acceptance stage prompt。
...
| `kind` | `stage` 等 |
| `stage` | `Product`、`Dev`、`QA`、`Acceptance`；Dev 的技术方案和实现通过 required_outputs 区分；非 stage prompt 可为空 |
...
| `source` | 记录点，例如 `runtime_driver.codex_exec` |
| `response_path` | 对应 last message 或 result bundle 路径；没有则为空 |
| `runtime_trace_path` | stage run trace 路径；非 stage prompt 或没有 run 时为空 |
...
对于 stage prompt，`runtime_trace_path` 应指向现有 stage-run 的 `<run_id>_trace.json`。实际 skill 注入快照保存在 trace 的 `steps[].details.skill_injection` 中；如果 prompt trace 也保存 `skills`，必须来自同一份 resolved snapshot，避免后续 skill 文件或 preference 变化影响回溯结果。
...
- `agent_team/runtime_driver.py`
  ...
  - 记录 Product / Dev technical plan / Dev / QA / Acceptance 的 `codex-exec` prompt。
  - 将 `prompt_path` 和 `prompt_id` 写进 stage run trace details。
  - 将 `runtime_trace_path` 和 resolved skills 写进同一条 prompt trace，或至少保证可通过 stage run trace 回溯。
  ...
  "prompts": [
    {
      ...
      "sha256": "...",
      "bytes": 12345,
      "runtime_trace_path": "/.../.agent-team/_runtime/sessions/<session_id>/roles/development/attempt-001/stage-results/development-runtime-trace.json",
      "skills": [
        {
          "name": "plan",
          "source": "builtin",
...
- 提供 stage run trace 入口；如果没有注入 skill，则展示为空状态而不是隐藏字段。
...
- `tests/test_prompt_traces.py`
  ...
  - `skills` 与 `runtime_trace_path` 会跟 prompt 一起写入元数据，或可通过 stage run trace 回溯。
