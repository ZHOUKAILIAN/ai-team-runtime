# AI_Team Enforced Stage Gates Design

Date: 2026-04-17

## Goal

Strengthen AI_Team from a workflow that can control stage order into a runtime that can enforce stage completion. The runtime must own state truth, evidence requirements, and transition eligibility instead of trusting worker claims.
...
The result should preserve the current AI_Team architecture:

- `ai-team` remains the controller
...
### 1. Add `ai-team acquire-stage-run`
...
### 2. Change `ai-team submit-stage-result`
...
### 3. Add `ai-team verify-stage-result`
...
```json
{
  ...
  "contract_path": "/repo/.ai-team/session-001/contracts/dev.json"
}
```
