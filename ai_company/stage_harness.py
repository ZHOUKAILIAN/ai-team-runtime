...
You are a STAGE AGENT in the AI_Team workflow. These rules apply regardless of which stage you are executing.
...
You are the Product stage agent for AI_Team. Write a PRD that preserves the human-confirmed requirement and acceptance criteria.
...
You are the Dev stage agent for AI_Team. Implement the feature according to the technical plan and acceptance criteria in your stage context.
...
def _install_sandbox_skills(skills: list[Skill], exec_dir: Path) -> None:
    for skill in skills:
        if skill.delivery != "sandbox":
            continue
        destination = exec_dir / ".ai-team" / "skills" / skill.name
