...
class SkillPackageTests(unittest.TestCase):
    ...
    def test_root_skill_describes_single_session_state_machine(self) -> None:
        ...
        self.assertIn("./scripts/agent-team-init.sh", content)
        self.assertIn("./scripts/agent-team-run.sh", content)
    ...
    def test_installable_skill_exists_and_declares_trigger(self) -> None:
        ...
        self.assertIn("/agent-team-run", content)
        self.assertIn("agent-team-run.sh", content)
        self.assertIn("scripts/", content)
        self.assertIn("Intake", content)
        self.assertIn("ProductDraft", content)
        self.assertIn("WaitForCEOApproval", content)
    ...
    def test_install_script_copies_skill_into_codex_home(self) -> None:
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            installed_helper = Path(temp_dir) / ".codex" / "skills" / "agent-team-workflow" / "scripts" / "agent-team-run.sh"
    ...
    def test_packaged_agent_team_run_helper_uses_current_workspace_repo_root(self) -> None:
        repo_root = Path(__file__).resolve().parents[1]
    ...
    def test_installed_helper_script_runs_from_vendored_runtime(self) -> None:
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            run_result = subprocess.run(
                [str(helper_script), "执行这个需求：做一个全局安装后可直接触发的 Agent Team 流程"],
                cwd=Path(temp_dir),
                env=env,
                capture_output=True,
                text=True,
